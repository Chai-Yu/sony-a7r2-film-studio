...
| 徕卡官方 Look LUT 与 L-Log 参考手册 | 归 Leica Camera AG 及相应权利人所有；不分发原始 LUT 与手册；APK 内嵌拟合参数。徕卡未提供中性参照，基准渲染由本项目构造，仍不构成徕卡的授权或背书 |
...
本工具参考富士公开的 GFX ETERNA 55 LUT；**公开下载不等于公开授权改编或再分发**。[富士网站条款](https://global.fujifilm.com/en/terms)有下载用途和改动限制，当前未确认针对本工具用途的独立授权。这是核对许可的起点，不是对具体地区法律适用的最终结论。自行构建前，也须确认基础 APK 和 LUT 的相应用途获准。徕卡方面同理：其 Look LUT 与 L-Log 手册同样只作参考，公开下载不等于公开授权；且徕卡不提供中性参照渲染，本案的基准由本项目按 L-Log 规范构造，因此那部分是我们自己的构建，不是徕卡的输出。

项目名称和风格名称仅用于识别参考来源及兼容性。Sony、FUJIFILM、Ricoh、Leica 及产品名称、Look 名称的商标权属于各自权利人。本项目未经这些公司认证、赞助或背书。
...
| Official Leica Look LUTs and the L-Log reference manual | Owned by Leica Camera AG and respective rights holders. Original LUTs and the manual are not distributed; fitted parameters are embedded in the APK. Leica provides no neutral reference, so the baseline rendering is constructed by this project and is neither Leica's output nor an endorsement |
...
| ライカ公式 Look LUT と L-Log リファレンスマニュアル | Leica Camera AG および各権利者に帰属。原本 LUT とマニュアルは配布せず、近似パラメータは APK に内蔵します。ライカは中性参照を提供していないため、基準レンダリングは本プロジェクトの構成物であり、ライカの出力でも推奨でもありません |
...
色彩の参照元は公開ダウンロード可能な GFX ETERNA 55 LUT です。ただし、公開されていることと改変・再配布の許諾は別です。[富士フイルムのサイト利用条件](https://global.fujifilm.com/en/terms)にはダウンロード用途や改変の制限があり、本ツールの用途に対する個別の許諾は未確認です。ビルド前に、基礎 APK と LUT の利用に必要な権利を確認してください。ライカも同様です。Look LUT と L-Log マニュアルは参照のみで、公開は許諾を意味せず、中性参照レンダリングが提供されていないため、本プロジェクトの基準はライカの出力ではなく独自の構成物です。特定地域の法律について最終判断を示すものではありません。

企業名・製品名・フィルム名は互換性と参照元の説明に使用します。商標は各権利者に帰属し、本プロジェクトは Sony、FUJIFILM、Ricoh、Leica の認証・支援・推奨を受けていません。
