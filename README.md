...
- 滤镜共 **17 个**：10 个富士官方 LUT 参考风格、5 个上游理光／街头风格、2 个徕卡 Look 参考风格。相机菜单以「富士」「理光」「徕卡」前缀区分。
- 包名与签名沿用旧版，可直接覆盖安装。更早的 [0.1.3-alpha](https://github.com/ukiki0718-netizen/sony-a5100-film-studio/releases/tag/v0.1.3-alpha)（上游仓库）仍保留供回退。

各版本的实测数据见[验证说明](docs/VALIDATION.md)。
...
| **a7R II / ILCE-7RM2，Android 4.1.2 / API 16** | 已实测安装、启动，以及「确认键打开滤镜菜单 → 按 MENU 返回拍摄」的往返操作（2026-09-14）。该机身的 `isRGBMatrixSupported()` 与 `isExtendedGammaTableSupported()` 均返回 false，但实测**两者都会被采纳**；本应用因此**无条件写入 RGB 矩阵与扩展 Gamma 表**，与上游理光模组一致。它**不采纳** `setColorMode()` 与 `setPictureEffect()` 的写入，所以 `applyNative()` 的机身风格降级在该机型上不起作用。照片／录像保存与色彩未在该机型逐项验证 |
...
- 拍照及录像待机时，按中心键进入滤镜选择。
- MENU 首页 →「滤镜强度」：**30% / 50% / 70% / 100%**。初始为 70%，拍照与录像共用，正常退出后保存选择。**100% 即该滤镜的完整 look**，向下同时减弱色彩与明暗。
...
- 每个风格由**一个 3×3 颜色矩阵**加**一条 1024 点影调曲线**构成，写入相机的 RGB 矩阵与扩展 Gamma 表。**两者必须成对生效**：矩阵按它拟合时依据的那条曲线标定，单独应用不构成该 look。
- 参数按富士公开 LUT 拟合。LUT 的输入是 F-Log2 / F-Gamut，不能直接套在普通索尼画面上，因此以 **WDR-709 作替代中性参考**，拟合出普通画面可用的矩阵与曲线。
- 富士家族的**色度**另外按 a7R II 的实拍标定：五个场景，每个都拍机身 Neutral、机身 STD 与应用输出三张，据此校正矩阵的色度响应。因此 100% 强度下更接近官方 look。
- 徕卡只发布 look LUT、没有中性参照，其基准渲染由本项目按 L-Log 规范构造。理光系列的参数取自上游项目，未作改动。
...
- **a7R II / ILCE-7RM2，Android 4.1.2 / API 16**：机身的能力查询报告不支持上述两项，但实测两者都会被采纳，因此同样使用本项目拟合的参数；Creative Style 与 Picture Effect 的写入不被采纳，见[实时取景说明](docs/LIVE-PREVIEW.zh-CN.md)。
...
- 0.2.2（a7R II，2026-09-14）：安装、启动，以及「确认键打开滤镜菜单 → 按 MENU 返回拍摄界面」连续数十次往返全部正常。同日按五组实拍三连照完成富士家族的色度标定。a5100 未逐项重测；两机的照片／录像保存与色彩均未在本版验证。

**应用内回放目前只显示照片。** 查看录像请退出应用，进入原机回放，并选择与文件对应的 XAVC S / AVCHD / MP4 观看模式。

**数值近似的边界：** 这不是富士机内胶片模拟的完整移植，也不模拟颗粒或传感器响应。a5100 未做 a7R II 那样的实拍色度标定；个别风格与官方 LUT 的偏差较明显。
