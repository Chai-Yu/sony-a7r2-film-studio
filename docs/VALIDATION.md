...
```text
5a6417043e9fe9bba470f7d10d61440c1227a10ba835734748b325e43c752ad3
```

`FilmStudio-0.2.2-alpha-movie-leica-anchor.apk`（anchor）— SHA-256:

```text
c5ebc6799258dfbc091657902bbb593e01ad7d1d737738d9a469a211e7e2f0c2
```
...
| 富士 ACROS | 0.000 | **0.000 —— 不参与标定**（见下） |

**注意 Reala Ace 的方向仍然是错的**：0.614 → 0.460 是**进一步去饱和**，而它需要的恰好相反
（见上面「已知未解决的一类问题」）。这是标量修法唯一尚未解决的**系统性**缺陷，而不是随机误差。

**黑白矩阵不参与标定（ACROS）。** ACROS 这类矩阵的三行完全相同，输出永远是灰的，
**没有色度可修**；而缩放并不会“放过”它——它动的是**灰度权重**，而那正是这个矩阵唯一在做的事。
实测：在饱和色样本上缩放后，渲染出的灰度平均挪动 **0.85%**、最多 **3.6%**。
而留一场景数据说**不动它反而更准**（RGB 误差 0.0681 / 0.0860 / 0.0317，缩放后为 0.0680 / 0.0861 / 0.0334）。
因此工具会识别“三行相同”的矩阵并跳过（不硬编码 id），`domain_calibration` 块里有 `monochrome` 字段记录这一点。
