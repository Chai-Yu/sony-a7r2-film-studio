...
- **白点アンカー**版 `FilmStudio-0.2.2-alpha-movie-leica-anchor.apk`（3,790,893 バイト、SHA-256 `18c6f068d253edf5d0fd417da9db160551a1ed43566626fa53e7dbe9702f98e4`）も同ページにあります。白を白のまま保ち、全体を暗くしません。**両版の違いはライカのトーン出力のみ**で、行列は共通です。
...
→ **[日本語インストール手順](docs/INSTALL.ja.md)**：入力資料の準備 → ローカルビルド → USB（pmca-gui）／Wi-Fi ADB インストール → 操作 → 更新・トラブル対応。
...
人物撮影では、まず30%と70%を比較してください。カラー系の強度は色変換と明暗カーブを弱めるもので、顔検出や肌色の自動補正ではありません。**ACROS、リコーのハイコントラスト白黒、森山風はどの強度でも色を持ちません。強度が下げるのはコントラストだけで、100%がそのフィルター本来のコントラストです。**
...
- **a7R II / ILCE-7RM2、Android 4.1.2 / API 16**：上記2項目に非対応と報告するため、Creative Style + ネイティブ Picture Effect の近似（[ライブビューの説明](docs/LIVE-PREVIEW.zh-CN.md)）へ自動的に降格し、近似パラメータは使用しません。
...
- 2026-09-14（a7R II）：インストール、起動、および「中央ボタンでフィルター選択を開く → MENU で撮影画面に戻る」の往復を数十回行い、すべて正常でした。この経路は以前アプリの状態管理を固まらせ（画面は閉じたがメニュー状態が残る）、以降どのキーにも反応せず電源再投入が必要でしたが、修正済みです。a5100 では本修正を再確認しておらず、両機種とも本版の保存ファイルと色は未検証です。

**アプリ内の再生画面には現在、写真だけが表示されます。** 動画はアプリを終了し、カメラ標準の再生画面で対応する XAVC S / AVCHD / MP4 の表示モードを選んでください。[検証記録](docs/VALIDATION.md)も参照してください。

富士フイルムのフィルムシミュレーションを完全移植したものではありません。F-Log2/F-Gamut LUT を通常のソニー映像へ直接適用せず、WDR-709 を代替の中性基準として3×3行列と1024点の共通カーブに近似します。**彩度特性は a7R II の実写 5 シーン（各シーンとも機体 Neutral／機体 STD／アプリ出力、うち 2 シーンは 100% 強度）で校正しています**。100% 強度での参照 look との残差は約 7.5% まで下がりました（未校正は 27%）。a5100 は未校正で、粒状感やセンサー特性は再現しません。近似誤差が大きいスタイルもあります。
